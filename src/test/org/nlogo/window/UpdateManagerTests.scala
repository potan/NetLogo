...
class UpdateManagerTests extends FunSuite {
  ...
  test("tick-based: all speeds") {
    assert((-50 to 50).map(debugInfo(UpdateMode.TickBased, _)).mkString("\n", "\n", "\n")
           === tickBased)
  }

  test("continuous: all speeds") {
    assert((-50 to 50).map(debugInfo(UpdateMode.Continuous, _)).mkString("\n", "\n", "\n")
           === continuous)
  }
